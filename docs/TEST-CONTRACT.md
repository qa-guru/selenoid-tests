...
| Integration WD/PW/min | Advertised catalog versions **create**. Create failure on `qa_guru` / `github` is **fail**, not skip. `*-min` + `enableVNC`/`enableVideo`/`enableHAR` is HTTP **400** (no session — hub v3.0.16, not a DELETE SLA). PW `enableHAR` hub DELETE within **8s** on **full** images only. |

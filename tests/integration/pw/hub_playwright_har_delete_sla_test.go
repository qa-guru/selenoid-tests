...
func TestHubPlaywrightHarSession_HubDeleteReturnsWithinSLA(t *testing.T) {
	cfg := config.MustLoad()
	allurex.Run(t, allurex.Meta{
		Name:      "Hub DELETE of Playwright enableHAR session returns within 8s",
		Package:   "tests.integration.HubPlaywrightHarDeleteSlaTests",
		Layer:     "integration",
		Component: "selenoid",
		Epic:      "selenoid",
		Feature:   "Playwright session delete",
		Story:     "enableHAR delete must not wait on CDP",
		Suite:     "Playwright hub HAR delete SLA",
		Browser:   allurex.BrowserChromium,
		Tags:      []string{"integration", "positive"},
	}, func(a *allurex.A) {
		assertPlaywrightHarDeleteWithinSLA(t, a, cfg, "")
	})
}

func assertPlaywrightHarDeleteWithinSLA(t *testing.T, a *allurex.A, cfg *config.Config, wsEndpoint string) {
	t.Helper()
	...
	a.Step("Connect Playwright with enableHAR", func() {
	...
	a.Step("Read new hub session id", func() {
		...
		require.NoError(t, err)
		require.NotEmpty(t, sessionID, "hub status must expose the Playwright session id (/hub/status, not public /status)")
	})

	a.Step("Hub DELETE within HAR SLA", func() {
